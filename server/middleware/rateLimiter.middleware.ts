...
export const enhancementLimiter = rateLimit({
  ...
  handler: (req, res) => {
    res.status(429).json({
      success: false,
      message:
        "Too many enhancement requests. Please wait before trying again.",
    });
  },
});

// Rate limiter for problem generation (3 requests per minute)
export const problemGenerationLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 3,
  message: "Too many problem generation requests",
  standardHeaders: true,
  legacyHeaders: false,
  validate: {
    ip: false,
  ...
  handler: (req, res) => {
    res.status(429).json({
      success: false,
      message:
        "Too many problem generation requests. Please wait before generating another problem.",
    });
  },
});
