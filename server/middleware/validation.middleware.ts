...
export const validateTemplateCreate = [
  ...
  body("category")
    ...
    .withMessage(
      "Category must be one of: code, content, analysis, creative, custom",
    ),
  ...
  body("variables.*.type")
    ...
    .withMessage(
      "Variable type must be one of: text, textarea, select, number",
    ),
...
export const validateProblemGeneration = [
  query("focus_area")
    .notEmpty()
    .withMessage("focus_area is required")
    .isIn(["arrays", "strings", "hash-tables", "linked-lists", "trees"])
    .withMessage(
      "focus_area must be one of: arrays, strings, hash-tables, linked-lists, trees",
    ),
  query("difficulty")
    .optional()
